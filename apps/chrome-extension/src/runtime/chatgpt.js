...
async function runChatGptPromptOnce(prompt, options = {}) {
  ...
  try {
    ...
    try {
      ...
      logError('ChatGptAutomation', 'Script injection failed.', { promptLabel, tabId, targetUrl, message });
      throw new Error(`Failed to inject ChatGPT prompt runner into ${targetUrl}: ${message}`);
    }
...
export async function runChatGptPrompt(prompt, options = {}) {
  const firstResult = await runChatGptPromptOnce(prompt, options);
  ...
  const retryResult = await runChatGptPromptOnce(prompt, {
    ...options,
    disableRetry: true,
    ...
    warmupDelayMs: Math.max(options.warmupDelayMs ?? 0, 2500),
  });
