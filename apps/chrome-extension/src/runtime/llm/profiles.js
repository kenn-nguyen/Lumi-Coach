...
function toProfileMap(profileList) {
  return Object.fromEntries(profileList.map((profile) => [profile.id, cloneValue(profile)]));
}
...
export function mergeLlmSettings(storedSettings, legacyChatGptTargetUrl = '') {
  ...
  if (storedSettings && typeof storedSettings === 'object') {
    const storedProfiles = storedSettings.profiles && typeof storedSettings.profiles === 'object'
    ...
    for (const [profileId, baseProfile] of Object.entries(defaults.profiles)) {
      ...
      merged.profiles[profileId] = storedProfile && typeof storedProfile === 'object'
        ? { ...baseProfile, ...cloneValue(storedProfile) }
        : cloneValue(baseProfile);
    }

    if (typeof storedSettings.activeProfileId === 'string' && storedSettings.activeProfileId in merged.profiles) {
      merged.activeProfileId = storedSettings.activeProfileId;
    }
  }

  const normalizedLegacyUrl = typeof legacyChatGptTargetUrl === 'string'
...
export function updateLlmSettings(currentSettings, activeProfileId, profileUpdates) {
  const resolved = mergeLlmSettings(currentSettings);
  const nextActiveProfileId = typeof activeProfileId === 'string' && activeProfileId in resolved.profiles
    ? activeProfileId
  ...
  if (profileUpdates && typeof profileUpdates === 'object') {
    resolved.profiles[nextActiveProfileId] = {
      ...nextProfile,
      ...profileUpdates,
      id: nextProfile.id,
